...
def get_optimizaton_parameters(ydata, e_b):
        ...
        low_bound =[e_b['low_ke_bound'], e_b['low_peak_width_bound'],*low_intensity_bound, *low_bkg_params_bound]
        high_bound=[e_b['high_ke_bound'], e_b['high_peak_width_bound'],*high_intensity_bound, *high_bkg_params_bound]
        p0 = (np.array(high_bound) + np.array(low_bound))/2.
...
def get_energy_params_from_user_input(input_form):
    ref_peaks = np.array([ float(r) for r in input_form['reference_peaks'].replace(',','.').split('\n') if r.strip()])
    ...
    return {'num_peaks': len(ref_peaks),
...
def func(xx, *p):
    ...
    sig = p[1]; intensities = p[2:-2]; a = p[-2]; b = p[-1]
    
    y = 0
    for i in range(0, len(mu_s)):
        y = y + intensities[i] * np.exp(-np.power(xx - mu_s[i], 2.) / (2 * np.power(sig, 2.)))
    y = y + a*(xx-mu_s[0])+ b
    return y
...
def main():
    ...
    with open(input_file.path,'r') as fp:
        ...
        for dict_data in vamas_data['blocks']:
            ...
            output_files.append({"name": '{}_be.txt'.format(dict_data['block_identifier']),
    ...
    new_properties = {'phonton_energy': photon_energy,
